...
/* date:  18AUG2006
   ...
   date:  25MAR2013 - KVLB, ISIS, STFC Alterations for use with NI 488.2 on Windows, based on theinitial code */
...
#include <taskwd.h>
#include <initHooks.h>
//Windows and NI 488.2 driver includes
#include <windows.h>
#include "ni4882.h"
...
static asynStatus aconnect(void *pdrvPvt,asynUser *pasynUser);
...
static const char* gpib_error_string( int error );
...
int GpibBoardDriverConfig(char *name,int autoConnect,int boardIndex,double timeout,int priority)
{
    ...
    if(status==asynError){
	    printf("GpibBoardDriverConfig: Can't register option.\n");
	    return -1;
    }
